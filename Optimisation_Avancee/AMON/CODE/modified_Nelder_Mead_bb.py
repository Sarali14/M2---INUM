...
results_file = ROOT / "instances/2/nelder_mead_log_bb.txt"
out_path = ROOT / "CODE/optimized_nelder_mead_bb.txt"
...
# -------------------------------------------------------------------
# Penalized function for the (n+1)-th turbine only
# -------------------------------------------------------------------
def penalized_function_new_turbine(X_new, instance_path, lambd):
    """
    X_new: np.array([x_{n+1}, y_{n+1}])
    BASE_LAYOUT: global, length 2n
    """
    X_new = np.array(X_new, dtype=float)
    if X_new.shape[0] != 2:
        raise ValueError(
            "penalized_function_new_turbine expects a 2D vector [x_{n+1}, y_{n+1}]."
        )

    # Build full vector: [base_layout, x_{n+1}, y_{n+1}]
    full_X = np.concatenate([BASE_LAYOUT, X_new])

    try:
        EAP, spacing, placing = windfarm_eval.windfarm_eval(
            instance_path, full_X.tolist()
        )
    except ValueError:
