...
VU=(mu_boddy/DU)^0.5;   // vitesse de l'orbite GEO
TU=DU/VU;               // temps
...
detat=[ vx
        vy
       -mu/r^3*x-F/m0*cos(w_terre*t)   // -F/m0*cos(w*t) la pression de radiation suivant x
       -mu/r^3*y-F/m0*sin(w_terre*t)   // -F/m0*cos(w*t) la pression de radiation suivant y
       ]; 
endfunction
...
t0=0;
t=[0:T/20:T*4]/TU;// définition du vecteur de temps de simulation en multiple de la periode 
rtol = 1e-12; // tolérance relative
atol = 1e-12; // tolérance absolue
