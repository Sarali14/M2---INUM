...
public:
  CS1D(double h, std::size_t N):
    _h(h), _N(N), _grid{N,N}, _d(N), _A(_grid),_B(_grid),_rhs(N), _beta(0) {}
  ...
  void thomas_solver(){
    for (std::size_t i=0;i<2;++i){ //first loop to normalize the first 2 rows that will be used for computations
       _d[i]=rhs[i]/_A(i,i); //store the normalized rhs TEMPORARILY in the solution vector, to avoid allocating a new vector
    ...
    for (std::size_t i=2;i<_N;++i){ //forward pass
       ...
       _d[i]=w*(rhs[i]-_A(i,(i-1+_N)%_N)*_d[i-1]);
    ...
    w=1/(1-_A(0,(i+1)%_N)*_A(1,(i-1+_N)%_N);
    _d[0]=w*(_d[0]-_A(0,(i+1)%_N)*_d[1]);
    _A(0,(i-1+_N)%_N)=w*_A(0,(i-1+_N)%_N);
    _A(0,(i+1)%_N)=-w*_A(0,(i+1)%_N)*_A(1,(i+1)%_N);
...
public:
    ...
    CS1D_o2(double h, std::size_t N, double alpha) :  CS1D<T>(h,N) {
        this->_alpha=alpha;
        this->_a=1+2*(alpha);
        this->_b=0;
        this->_c=0;
...
public:
    ...
    CS1D_o4(double h, std::size_t N, double alpha) :  CS1D<T>(h,N) {
        this->_alpha=alpha;
        this->_a=(2/3)*(alpha+2);
...
public:
    ...
    CS1D_o6(double h, std::size_t N, double alpha) :  CS1D<T>(h,N) {
        this->_alpha=alpha;
        this->_a=(1/6)*(alpha+9);
...
public:
    ...
    CS1D_o8(double h, std::size_t N) :  CS1D<T>(h,N) {
        this->_alpha=3/8;
        this->_a=(2/3)*(alpha+2);
        this->_b=(1/3)*(4*alpha -1);
        this->_c=0;
