   ...
   function Phi = Phi_Pk(lmd1, lmd2, pk)
       lmd3 = 1.0 - lmd1  - lmd2
        ...
        case 3 then
          ...
          printf("ONLY P1, P2, P3 ARE IMPLIMENTED");
...
/* THIS WAS USED TO SAVE THE RESULTS FOR THE STABILIZED CONVECTION PROBLEM 
// ================================
// Save convergence results to CSV (NO ErrL2)
// ================================
fname = msprintf("conv_pb%d_P%d_i%d_m%d.csv", pb, EF_Pk, i, mExp);

fd = mopen(fname, "wt"); // write text (overwrites)
mfprintf(fd, "Nddl,OL1,OL2,OLinf\n");

for r = 2:Nconv
    k = r - 1; // because Ordre_de_Conv is length Nconv-1
    mfprintf(fd, "%d,%.16e,%.16e,%.16e\n", ...
             NddLC(r), ...
             Ordre_de_Conv(1,k), ...
             Ordre_de_Conv(2,k), ...
             Ordre_de_Conv(3,k));
